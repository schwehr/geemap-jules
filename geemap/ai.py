...
def fix_ee_python_code(
    code: str,
    ee: Any = None,
    geemap_instance: Any = None,
    model_name: str = "gemini-3-pro-preview",
    fix_true_false: bool = True,
    fix_null: bool = True,
    quote_list_content: bool = False,
) -> str:
    """Asks a model to do ee python code correction in the event of error.

    Fixes common errors in Earth Engine Python code generated by AI models.
    ...
    if quote_list_content:
        def quote_items(match):
            items = match.group(1).split(',')
            quoted_items = []
            for item in items:
                stripped = item.strip()
                if stripped and not (stripped.startswith("'") or stripped.startswith('"') or stripped.isdigit() or stripped.replace('.', '', 1).isdigit()):
                    quoted_items.append(f"'{stripped}'")
                else:
                    quoted_items.append(stripped)
            return '[' + ', '.join(quoted_items) + ']'
        code = re.sub(r'\[(.*?)\]', quote_items, code)

    if ee is None or geemap_instance is None:
        return code
